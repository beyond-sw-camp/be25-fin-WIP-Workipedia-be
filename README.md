...
### 문제 정의
현재 인사·온보딩 관련 정보는 메일, 메신저, 사내 게시판, PDF 매뉴얼 등 여러 채널에 분산되어 있어 필요한 정보를 빠르게 찾기 어렵다. 이로 인해 신규 입사자는 연차, 근태, 복지, 교육, 장비 지급과 같은 기본적인 정보 확인에도 많은 시간을 소모하게 되며, HR 담당자는 동일한 질문에 반복적으로 답변해야 하는 문제가 발생한다.
또한 부서별 온보딩 방식과 안내 수준이 달라 신규 입사자의 적응 경험에 차이가 생길 수 있고, 기존 문의 처리 이력이 체계적으로 축적되지 않아 유사한 문의가 반복되는 비효율도 존재한다.
기존 사내 시스템 역시 단순 문서 검색 중심으로 운영되는 경우가 많아 사용자가 여러 문서를 직접 확인해야 하며, 어떤 부서에 문의해야 하는지 알기 어려운 문제도 있다. 더불어 실제 문의 과정에서 생성된 유용한 답변이 FAQ나 조직 지식으로 재활용되지 못하고 일회성 대응으로 끝나는 한계가 존재한다.
본 프로젝트는 이러한 문제를 해결하기 위해, 분산된 HR 정보를 통합 검색하고 AI 기반 답변 및 문의 티켓 연계 기능을 제공하는 온보딩 지식 허브 구축을 목표로 한다.
