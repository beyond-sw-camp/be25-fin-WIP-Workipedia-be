...
Infrastructure 레포지토리는 실제 운영 환경을 관리합니다.

주요 역할은 다음과 같습니다.

Backend / Redis / Elasticsearch Docker Compose 관리
AI Docker Compose 관리
Qdrant Docker Compose 관리
서비스 Health Check 구성
Docker Network 분리
배포 Shell Script 관리
환경 변수(.env) 및 Secret 관리

또한 GitHub Actions를 통해 Docker Compose 문법을 자동 검증하여 운영 환경에 잘못된 설정이 반영되는 것을 방지합니다.
...
Frontend
Vue Build 결과를 AWS S3에 업로드
CloudFront를 통해 서비스 제공
Backend
...
Backend는 AI 서버를 직접 호출하지 않고 Internal ALB를 통해 AI Auto Scaling Group으로 요청을 전달하도록 구성했습니다.
