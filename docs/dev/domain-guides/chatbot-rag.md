...
사용자 질문에 대해 매뉴얼·워키·지식 RAG·Tool 후보를 통합 평가하고 출처가 있는 답변을 반환한다.

## 확정된 구조

후보 통합 순서: A/B/C/D 수집 → 후보 통합 → 최종 답변 생성

```text
A. 매뉴얼 RAG ─┐
B. 워키 RAG ───┼→ 후보 통합·평가 → 최종 답변
C. 지식 RAG ───┤
   - TEAM_ADMIN 승인 지식화 게시판(`KNOWLEDGE_DATA`)
   - SYSTEM_ADMIN 수기 지식(`MANUAL_KNOWLEDGE`)
D. READ_ONLY Tool ─┘

근거가 부족하면 Worki 질문 또는 요청 티켓 생성 전환 액션
```
...
- 후보 수집과 최종 답변 오케스트레이션은 AI 서버가 담당한다.
...
- 매뉴얼·워키·지식 RAG·Tool 후보 수집과 통합 답변 오케스트레이션
...
- `callerEmployeeId`는 인증된 사용자 정보에서 조회하며 사용자 질문이나 요청 body의 사번을 신뢰하지 않는다.
- AI 서버는 `callerEmployeeId`를 LLM prompt에 포함하지 않고 Tool 실행 요청 body로만 전달한다.

AI 응답의 핵심 필드는 `answer`, `sources`, `route`, `action`, `step_history`다. 통합 응답의 `route`는 `AGGREGATED`일 수 있다. 출처는 `sourceType`, `sourceId`, nullable `chunkIndex`, 제목과 점수를 포함한다. BE는 `route`가 아니라 `sources`를 기준으로 답변 출처를 저장하고, 매뉴얼 출처는 `(manualId, chunkIndex)`로 실제 청크를 식별한다. Tool 실행 결과는 AI 내부 근거로만 사용하고 `sources`에는 포함하지 않는다. `step_history`는 운영 로그 진단에 사용하며 사용자 메시지에는 저장하지 않는다. 활성 `custom_prompt`가 있으면 요청에 포함하고, 없거나 비활성이면 `null`을 전달한다.

AI 호출 실패 시 임의 답변을 생성하지 않고 서비스 이용 불가 안내를 반환한다. `CREATE_TICKET` 같은 전환 액션은 `next_action`으로 저장하며 일반 근거 답변과 구분한다.

챗봇 BE 클라이언트 timeout 기본값은 110초이며 RestClient와 스트리밍 WebClient에 동일하게 적용한다. 배포 프록시가 있는 환경에서는 Nginx 등 상위 프록시 timeout도 110초 이상으로 맞춘다.
...
2. 커밋 후 텍스트 계열 `@Scheduled` 워커가 AI 동기화 API를 호출한다.
3. AI가 원문을 chunking, embedding하여 Qdrant에 upsert한다.
4. 성공 시 `SYNCED`, 실패 시 `FAILED`와 실패 사유를 작업 테이블에 저장한다.
5. 실패 작업은 최대 5회 `retry_count^2`분 백오프로 자동 재시도한다.

| sourceType | 생성 시점 | operation |
|---|---|---|
| `WORKI` | 질문 생성·수정, 답변 생성·수정 | `UPSERT` |
| `KNOWLEDGE_DATA` | 지식 승인·수정 | `UPSERT` |
| `KNOWLEDGE_DATA` | 지식 삭제 | `DELETE` |

워키는 질문 ID를 `sourceId`로 사용한다. 답변 변경도 해당 질문 문서를 다시 구성해 인덱싱하도록 질문 ID 기준 `WORKI` 작업을 만든다.
...
- AI는 활성·승인된 `READ_ONLY` Tool과 허용 파라미터만 실행한다.
