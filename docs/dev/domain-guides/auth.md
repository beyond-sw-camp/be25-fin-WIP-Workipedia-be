...
| 내 발행 티켓 수정 | `PATCH /api/v1/me/tickets/{ticketId}` |
| 내 발행 티켓 삭제 | `DELETE /api/v1/me/tickets/{ticketId}` |
...
7. 상태가 `RECEIVED`, `ASSIGNED`, `IN_PROGRESS` 중 하나이고 48시간이 지나지 않은 경우에만 `editable`, `deletable`을 `true`로 반환한다.

### 5. 내 발행 티켓 수정

`PATCH /api/v1/me/tickets/{ticketId}`

1. Request Header의 Access Token으로 로그인 사용자를 식별한다.
2. `ticketId`와 로그인 사용자 ID를 함께 조건으로 사용하여 본인이 발행한 티켓만 수정 대상으로 조회한다.
3. 삭제된 티켓은 수정할 수 없다.
4. 티켓 상태가 `RECEIVED`, `ASSIGNED`, `IN_PROGRESS` 중 하나인지 확인한다.
5. 티켓 생성 시각 기준 48시간이 지나지 않았는지 확인한다.
6. 수정 가능 조건을 만족하면 티켓의 제목과 내용을 변경하고 수정 시각을 갱신한다.
7. 수정 가능 조건을 만족하지 않으면 `409 Conflict`를 반환한다.
8. 수정 성공 시 변경된 티켓 상세 정보를 반환한다.

### 6. 내 발행 티켓 삭제

`DELETE /api/v1/me/tickets/{ticketId}`

1. Request Header의 Access Token으로 로그인 사용자를 식별한다.
2. `ticketId`와 로그인 사용자 ID를 함께 조건으로 사용하여 본인이 발행한 티켓만 삭제 대상으로 조회한다.
3. 이미 삭제된 티켓은 삭제 대상에서 제외한다.
4. 티켓 상태가 `RECEIVED`, `ASSIGNED`, `IN_PROGRESS` 중 하나인지 확인한다.
5. 티켓 생성 시각 기준 48시간이 지나지 않았는지 확인한다.
6. 삭제 가능 조건을 만족하면 실제 row를 삭제하지 않고 soft delete 처리한다.
7. soft delete 시 `status`는 `DELETED`, `deletedAt`은 현재 시각, `isDeleted`는 `Y`로 변경한다.
8. 삭제 가능 조건을 만족하지 않으면 `409 Conflict`를 반환한다.
9. 삭제 성공 시 `204 No Content`를 반환한다.
