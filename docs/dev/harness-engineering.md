...
> 문서 유형: Quality Guide
> 상태: Draft
> 정본 위치: `docs/dev/harness-engineering.md`
> 관련 문서: `docs/reference/constitution.md`, `docs/reference/service-flow.md`, `docs/reference/prd.md`, `docs/reference/trd.md`
> 버전: v0.2
> 최종 수정: 2026-05-31
...
| P1 | 질문/요청 전환 | 챗봇 실패 또는 실제 처리 필요 시 워키/요청 티켓으로 이어지지 않음 |
...
| Privacy by Default | BE RDB의 PII가 암호화 저장되고 권한 있는 흐름에서만 복호화된다 | 단위/통합 테스트 |
| Privacy by Default | 출력 응답에 PII가 포함되면 거부 또는 마스킹된다 | 안전성 테스트 |
| Explicit Authority | 역할은 `USER`, `TEAM_ADMIN`, `SYSTEM_ADMIN`만 허용한다 | 도메인/DB 테스트 |
| Always a Knowledge Loop | 챗봇 실패 응답에는 워키 또는 요청 티켓 전환 액션이 포함된다 | API 계약 테스트 |
| Always a Knowledge Loop | 처리 완료 티켓은 TEAM_ADMIN 승인과 Vector Store 동기화 흐름을 가진다 | 워크플로우 테스트 |
...
| RAG-003 | 근거 부족 | 근거가 없으면 "찾지 못함" 계열 응답과 워키 또는 요청 티켓 액션을 반환한다 |
...
| ID | 검증 | 통과 기준 |
|---|---|---|
| SEC-001 | RDB 암호화 | 암호화 대상 개인정보가 DB에 평문으로 저장되지 않는다 |
| SEC-002 | AI 응답 마스킹 | 주민번호·카드번호가 사용자 응답에 원문으로 노출되지 않는다 |
| SEC-003 | 로그 보호 | 개인정보와 credential 원문이 애플리케이션·감사 로그에 남지 않는다 |
| SEC-004 | 일반 사용자 삭제 제한 | USER는 질문/답변 hard delete API에 실패한다 |
| SEC-005 | 관리자 로그 | TEAM_ADMIN/SYSTEM_ADMIN 작업 후 `admin_logs`에 actor/action/target/time이 남는다 |
...
| WF-004 | 요청 티켓 전환 | 실제 처리나 공식 확인이 필요한 입력은 요청 티켓으로 전환된다 |
| WF-005 | 부서 배정 감사 | 자동/수동 부서 배정 점수와 근거가 남는다 |
| WF-006 | 공통 접수 큐 | 라우팅 신뢰도 낮은 요청은 공통 접수 큐로 이동한다 |
| WF-007 | 팀원 배정 | TEAM_ADMIN이 자기 팀 티켓을 담당 팀원에게 배정할 수 있다 |
| WF-008 | 지식화 승인 | 처리 완료 티켓은 개인 정보를 제거하고 TEAM_ADMIN 승인 후 지식으로 등록될 수 있다 |
| WF-009 | 이관 권한 | TEAM_ADMIN만 티켓 이관을 요청할 수 있다 |
| WF-010 | 이관 큐 이동 | 이관 요청된 티켓은 공통 접수 큐로 이동한다 |
...
| user_role | USER, TEAM_ADMIN, SYSTEM_ADMIN | USER |
...
현재 프로젝트는 Spring Boot 3.5, Java 21, JPA, Redis, RabbitMQ, Spring Scheduler, Security, Flyway 기반이다. 하네스 테스트는 아래 패키지 구조로 시작한다.
...
- 답변 불가 질문 5개
- 개인정보 포함 질문 5개
- 워키 질문/요청 티켓 전환 케이스 9개
- 관리자 권한 질문 3개

### Day 3: 정책 하네스 구현

- 개인정보 마스킹 테스트
- USER/TEAM_ADMIN/SYSTEM_ADMIN 권한 테스트
...
- 질문/요청 -> 티켓 전환
- 공통 접수 큐 -> 부서 배정 -> 팀원 배정
- TEAM_ADMIN 이관 요청 -> 공통 접수 큐 -> SYSTEM_ADMIN 재배정
- 처리 완료 티켓 -> 지식화 생성/승인/동기화
- 채택/포인트 기본 검증
...
- 챗봇 실패 흐름이면 워키/요청 티켓 전환 검증이 있다.
- 티켓 처리 완료 흐름이면 지식화 승인과 동기화 상태 검증이 있다.
...
| PII 보호 정책 | 어떤 필드를 암호화하고 어떤 응답 패턴을 마스킹할지 | RDB 암호화 + 사용자에게 반환하는 AI 응답 마스킹 |
| 티켓 부서 배정 | 라우팅 점수, 공통 접수 큐, 감사 필드 | rule 기반 먼저, LLM은 보조 |
