...
| M2 | 6/15 ~ 6/21 | A→B→C→D 폴백 전환, 승인 지식 동기화, Qdrant 검색 |
...
- [ ] 사용자별 대화 이력이 조회된다
- [ ] 고객사별 Embedding provider와 Qdrant 기반 검색이 동작한다
- [ ] RAG 실패 시 매뉴얼→워키→지식 RAG→Tool 순서로 전환한다
- [ ] 지식 RAG는 `KNOWLEDGE_DATA`와 `MANUAL_KNOWLEDGE`를 분리 저장하고 검색 결과만 통합한다
- [ ] 챗봇 답변에 출처가 포함된다
