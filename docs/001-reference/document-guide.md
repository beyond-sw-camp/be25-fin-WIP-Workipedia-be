...
| Handoffs | `../998-handoffs/` | 휴가/부재/담당 변경 시 변경사항 인수인계 |
| Domain Development Guide | `../010-development/domain-guides/overview.md` | 도메인별 개발 진입 기준과 참고 문서 |
| Midterm Guide | `../999-presentation/midterm-presentation-guide.md` | 중간 발표 메시지와 시연 구성 |
...
| 998 | `998-handoffs` | 휴가/부재/담당 변경 인수인계. 상시 개발 기준 문서보다 후순위로 참고 |
| 999 | `999-presentation` | 발표 메시지와 시연 흐름. 개발 기준 문서보다 후순위로 참고 |
...
7. `../003-adr/001-database-choice.md`
8. `../003-adr/002-rag-strategy.md`
9. `../003-adr/003-auth-strategy.md`
10. `../003-adr/004-ticket-routing-strategy.md`
11. `../003-adr/005-role-permission-strategy.md`
12. `../003-adr/006-knowledge-conversion-strategy.md`
13. `../003-adr/007-notification-strategy.md`
14. `../003-adr/008-local-llm-security-strategy.md`
...
- ADR은 `001`, `002`처럼 3자리 번호를 유지하고, 새 결정 문서는 마지막 번호 다음으로 추가한다.
- 질문/요청/티켓/지식화 흐름 변경은 `service-flow.md`에 먼저 반영한다.
- 제품 범위 변경은 `prd.md`에 먼저 반영한 뒤 구현한다.
- 기술 구조, 테이블, API 변경은 `trd.md` 또는 별도 기술 문서에 반영한다.
- 당일 작업 시작점은 `../006-planning/daily-plans/YYYY-MM-DD.md`로 둔다.
- daily plan은 날짜별 파일로 작성하고, 이전 날짜 파일을 덮어쓰지 않는다.
...
| `../999-presentation/midterm-presentation-guide.md` | 중간 발표 메시지와 시연 구조 |
