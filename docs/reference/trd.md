...
1. **인덱싱(`ai_sync_jobs` + 일일 정합성 점검)** — KNOIT_006
   ...
   - 작업은 `MANUAL`, `WORKI`, `KNOWLEDGE_DATA`, `MANUAL_KNOWLEDGE`, `DEPT_RR` sourceType과 `UPSERT`/`DELETE` operation으로 구분
...
2. **질의 처리(실시간)** — KNOIT_001~003
   - 사용자 질문과 최근 세션 기록으로 검색용 독립 질문을 구성
   - 검색용 질문 임베딩 → Qdrant 유사도 검색(top-k)
   - 검색 후보를 Cross-Encoder로 재정렬
   - 검색된 chunk + 원본 매뉴얼/워키 메타 → LLM 프롬프트 컨텍스트 구성
   - LLM 응답 생성 후 사용자에게 반환할 답변만 마스킹하고 출처 메타와 함께 반환(KNOIT_003/007)
...
5. **지식화 발행과 Vector Store 동기화**
   ...
   - AI 서버가 원문을 chunking, embedding하여 Qdrant에 upsert
   - 작업 상태는 `PENDING`, `PROCESSING`, `SYNCED`, `FAILED`로 관리하고 실패 건을 최대 5회 재시도
...
| `ai_sync_jobs` | RDB 변경과 같은 트랜잭션으로 기록하는 AI 동기화 작업, 재시도·멱등성·오류 상태 관리 |
...
- `target_type` (WORKI_QUESTION)
- `target_id`, `reaction_type` (LIKE)
- 사용자와 대상 조합은 unique
...
- `routing_log_id` PK
- `ticket_id` FK → tickets
- `recommended_department_id` FK → departments NULL 허용
- `top_score`, `score_margin` DECIMAL
...
이 테이블은 AI 응답을 받은 시점의 추천 내역을 변경 없이 보존한다. 이관으로 변경되는 `tickets.assigned_department_id`를 최초 추천 부서 기록으로 사용하지 않는다.
...
#### ai_sync_jobs
- `ai_sync_job_id` PK
- `source_type` (MANUAL / WORKI / KNOWLEDGE_DATA / MANUAL_KNOWLEDGE / DEPT_RR)
- `source_id` BIGINT
- `operation` (UPSERT / DELETE)
- `status` (PENDING / PROCESSING / SYNCED / FAILED), 기본 PENDING
- `retry_count`, `last_error`, `next_retry_at`
- `lease_expires_at`, `started_at`, `completed_at`
- 시간컬럼, soft delete 컬럼
- `idx_ai_sync_jobs_status_next_retry`로 실행 가능한 작업을 찾고, `idx_ai_sync_jobs_source`로 동일 source의 최신 완료 작업을 판별한다.

`AiSyncJob`은 `BaseTimeEntity`를 상속한다. 워커는 native query의 `FOR UPDATE SKIP LOCKED`로 배치 단위 작업을 선점하고, `lease_expires_at`이 지난 `PROCESSING` 작업은 `PENDING`으로 되돌린다. 실패 시 `retry_count^2`분 백오프로 재시도하며 5회 실패하면 `FAILED`로 고정한다.
...
| `ticket_routing_logs` 보강 컬럼 | 추천 부서, `top_score`, `score_margin`, 모델/provider 버전 | 신규 migration |
...
| POST | `/worki/questions/{id}/like` | 좋아요 |
| DELETE | `/worki/questions/{id}/like` | 좋아요 취소 |
...
| RDB 개인정보 보호 | BE가 민감정보를 암호화 저장하고 권한이 있는 사용 흐름에서만 복호화 |
| AI 응답 마스킹 | KNOIT_007/008 — 사용자에게 반환하는 최종 LLM 응답에서 주민번호·카드번호 등을 마스킹하고 실패 시 차단 |
...
| 로깅 | 챗봇 처리 상태·지연·출처 식별자, 관리자 작업, 인증 이벤트 기록. 개인정보 원문과 credential은 제외 |
...
- `POST /api/v1/chat`: 현재 질문, 활성 `customPrompt`, 최근 세션 메시지를 받아 답변·출처·route·action·step history를 반환한다.
- `POST /api/v1/tickets/routing`: 티켓을 부서 R&R·승인 사례와 비교해 배정 결정과 후보 Top 3를 반환한다.
- `POST /api/v1/department/routing-prompt`: 자연어 지시와 현재 부서 R&R을 받아 변경 대상 부서만 반환한다.
- `POST /api/v1/knowledge/sync`, `DELETE /api/v1/knowledge/{source_id}`: 부서 R&R과 승인 사례를 단건 동기화·삭제한다.
- `POST /api/v1/documents/ingest`: 파일 매뉴얼을 multipart로 인덱싱한다.
- `POST /api/v1/documents/ingest-text`: 워키·승인 지식·수기 지식과 직접입력 매뉴얼을 JSON 텍스트로 인덱싱한다.
- `DELETE /api/v1/documents/{source_id}`: 매뉴얼·워키·승인 지식·수기 지식을 삭제한다.
- 동일 source의 작업 순서, 재시도와 최종 상태는 BE `ai_sync_jobs`가 관리하고 AI는 deterministic point ID 기반 멱등 저장을 담당한다.
