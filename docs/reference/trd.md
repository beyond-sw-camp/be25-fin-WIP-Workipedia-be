...
| `ai_sync_jobs` | RDB 변경과 같은 트랜잭션으로 기록하는 AI 동기화 작업, 재시도·멱등성·오류 상태 관리 | 신규 migration |
...
- `POST /api/v1/documents/ingest`, `DELETE /api/v1/documents/{source_id}`: 매뉴얼·워키·승인 지식·수기 지식을 인덱싱·삭제한다.
