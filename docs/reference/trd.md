...
> 관련 문서: `docs/reference/constitution.md`, `docs/reference/prd.md`
>
> 버전: v0.1 (Draft)
> 최종 수정: 2026-05-28
...
3. **실패 / 불만족 흐름** — KNOIT_005
   - LLM이 답변 불가 또는 사용자 불만족 피드백 → 워키 질문 등록 흐름으로 분기, 챗봇 입력 내용을 질문 초안으로 전달(WORKI_003)
...
- `role` VARCHAR(20) CHECK IN ('USER','ADMIN'), 기본 USER
...
- `user_id` (요청자), `assigned_department_id`
- `status` (처리대기 / 이관 요청 / 이관 완료 / 처리완료 / 반려 / 삭제)
- `due_at`

#### ticket_transfer_requests
- `request_id` PK
- `ticket_id` FK → tickets
- `from_department_id`, `target_department_id`
...
- `action_type` CHECK IN ('USER_DEACTIVATE','WORKI_READ','WORKI_UPDATE','WORKI_DELETE','MANUAL_UPDATE','MANUAL_DELETE', ...)
...
| POST | `/tickets/{id}/transfer-requests` | 티켓 이관 요청 |
| PATCH | `/tickets/{id}/transfer-requests/{requestId}` | 티켓 이관 승인/반려 |
...
3. 티켓 자동 배정 알고리즘 — 키워드 기반 vs LLM 분류기 vs 부서 임베딩 매칭
